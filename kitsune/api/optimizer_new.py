...
from ..core.compiler import CompilerConfig, TorchCompiler
...
class KitsuneOptimizer:
        ...
        try:
            ...
            if self.config.use_tf32 and torch.cuda.is_available():
                ...
                if compute_cap[0] >= 8:  # Ampere or newer
                    ...
                    logger.info(
                        f"✓ TF32 precision enabled (GPU: SM{compute_cap[0]}{compute_cap[1]})"
                    )
                else:
                    logger.info(
                        f"⏭️  TF32 not available (GPU: SM{compute_cap[0]}{compute_cap[1]} - requires SM80+)"
                    )
    ...
    def _setup_cuda_graphs(self):
        ...
        try:
            graph_config = GraphConfig(enabled=True, warmup_iters=self.config.graph_warmup_iters)

            self.graph_capture = CUDAGraphCapture(graph_config)

            # Capture graph
            success = self.graph_capture.capture(self.optimized_model, self.sample_input)
    ...
    def __call__(self, input_data: torch.Tensor) -> torch.Tensor:
        ...
        if (
            self.graph_capture is not None
            and self.graph_capture.is_captured
            and tuple(input_data.shape) == self.graph_capture.input_shape
        ):
