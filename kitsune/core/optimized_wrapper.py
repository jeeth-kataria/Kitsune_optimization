...
class OptimizedModelWrapper(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self._graph_captured and not self.training and x.shape == self._static_input.shape:
...
def create_optimized_model(
    ...
    return OptimizedModelWrapper(
        model, sample_input, use_cuda_graphs=use_cuda_graphs, compile_mode=compile_mode
    )
