...
class RTXSparsityOptimizer:
    ...
    def apply_sparse_pruning(self, model: nn.Module, target_sparsity: float = 0.5) -> nn.Module:
        """
...
class RTXCUDAGraphOptimizer:
    ...
    def capture_graph(
        self, model: nn.Module, sample_input: torch.Tensor, warmup_runs: int = 3
    ) -> Callable:
        """
...
class RTXCompileOptimizer:
    ...
    def compile_model(
        self, model: nn.Module, mode: str = "reduce-overhead", fullgraph: bool = False
    ) -> nn.Module:
        """
...
class RTXOptimizer:
    ...
    def _log_gpu_info(self):
        ...
        if self.gpu_info.tier != RTXTier.UNKNOWN:
            ...
            logger.info(
                f"   Compute: SM{self.gpu_info.compute_capability[0]}{self.gpu_info.compute_capability[1]}"
            )
    ...
    def optimize(
        ...
        if level == RTXOptimizationLevel.CONSERVATIVE:
            ...
            if self.compile_optimizer.is_available:
                model = self.compile_optimizer.compile_model(model, mode="reduce-overhead")
            ...
            if self.compile_optimizer.is_available:
                model = self.compile_optimizer.compile_model(model, mode="max-autotune")
        ...
        if enable_cuda_graphs:
            try:
                graph_forward = self.cuda_graph_optimizer.capture_graph(model, sample_input)
...
def optimize_for_rtx(
    model: nn.Module, sample_input: torch.Tensor, level: str = "balanced"
) -> nn.Module:
    """
