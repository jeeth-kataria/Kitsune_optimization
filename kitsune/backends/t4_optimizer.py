...
class T4QuantizationOptimizer:
    ...
    def validate_accuracy(
        ...
        with torch.no_grad():
            # Get outputs
            original_out = original_model(
                test_input.cuda() if next(original_model.parameters()).is_cuda else test_input
            )
            ...
            logger.info(
                f"   Accuracy validation: max_diff={max_diff:.6f}, mean_diff={mean_diff:.6f}"
            )
...
class T4Optimizer:
    ...
    def benchmark(
        ...
        return self.jit_optimizer.benchmark(
            original_model, optimized_result.model, sample_input, iterations
        )
...
def optimize_for_t4(
    ...
    result = optimizer.optimize(
        model, sample_input, level=level_map[level], calibration_data=calibration_data
    )
