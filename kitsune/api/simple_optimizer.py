...
class KitsuneOptimizer:
    ...
    def __init__(
        self, model: nn.Module, sample_input: torch.Tensor, config: Optional[KitsuneConfig] = None
    ):
        """
...
def optimize_model(
    ...
    config = KitsuneConfig(
        mode=mode, use_compile=use_compile, use_cuda_graphs=use_cuda_graphs, verbose=verbose
    )

    return KitsuneOptimizer(model, sample_input, config)


__all__ = ["KitsuneOptimizer", "KitsuneConfig", "optimize_model", "OptimizationMode"]
